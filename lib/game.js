...
Game.prototype.ballCollisionWithPaddle= function (){
  ...
  var paddleLeft = [paddle.x, (paddle.width/10)]
  var paddleRight = [(paddle.width - (paddle.width/10)), paddle.width]
  var paddleMain = [paddleLeft[1], paddleRight[0]];
...
Game.prototype.startScreen = function() {
    ...
    this.ctx.fillText('Use the left and right arrow keys to move the paddle', canvas.width/6, canvas.height/3);
    this.ctx.fillText("Click the screen to get breakin'", canvas.width/6, canvas.height/2);
  };
